 ...
 *      Author: Vincent
 *
 * GFX port Phase D: replaced "#include Model.h"/"ant.h" with the specific
 * globals needed (vue_global.h, att_global.h, Locator.h for `locator`,
 * SegmentManager.h for `segMngr`, debug_screen_data.h in place of `stc`,
 * g_structs.h for hrm_info/bsc_info/m_komoot_nav). Dropped
 * sysview_task_void_enter()/exit() SystemView profiling calls (SDK16
 * tracing, not ported, same as every other file in this port).
 ...
 *
 * tasksCRS()'s `vue.displayGPS()` became `this->displayGPS()`: the
 * original calls it on the global `vue` object out of habit (both `vue`
 * and `this` are literally the same object once the real, assembled Vue
 * class exists, since Vue itself inherits VueGPS the same way VueCRS
 * does here) -- but in this port `vue`'s type (VueBase) doesn't inherit
 * VueGPS at all (see vue_global.h), while `this` (VueCRS) already
 * virtually inherits it directly, so `this->displayGPS()` is the
 * available, behavior-identical equivalent -- the same substitution
 * VuePRC.cpp's own tasksPRC() already uses for its own eVuePRCScreenGps
 * case.
 *
 * afficheScreen2()'s `model_get_navigation(&navi)` call is dropped:
 * traced to Model.cpp's own implementation, it's a no-op unless
 * BLE_STACK_SUPPORT_REQD, in which case it forwards to ble_komoot_c --
 * out of scope for this port by the user's own 2026-09-06 decision (see
 * CLAUDE.md). Reads the already-declared, already-zero-initialized
 * global `m_komoot_nav` (g_structs.h/g_structs.c) directly instead of a
 * local `navi` copy -- an honest "no real komoot nav data flows yet",
 * not a fake stand-in, and the identical end state the original's own
 * no-op stub produces without BLE_STACK_SUPPORT_REQD.
 *
 * afficheSensors() is the one real, deliberate trim, not a straight
 * port: its entire body is FXOS8700 sensor-fusion output
 * (fxos_get_yaw()/fxos_get_pitch()/fxos_histo_size()/fxos_histo_read()/
 * fxos_get_roughness()) plus a barometer roughness reading
 * (baro.getRoughness()) -- none of it ported (Attitude.cpp, the class
 * that owns FXOS fusion, is explicitly not part of this port yet, per
 * CLAUDE.md's Phase 1 note), so there is no real data source to read
 * here at all, unlike tasksPRC()'s Boucle*-only trim. Replaced with a
 * placeholder screen stating this plainly, rather than either fabricating
 * values or leaving the page blank with no explanation.
 */
...
void VueCRS::afficheScreen1(void) {
	...
	case eVueCRSScreenDataFull:
	{
		...
		this->cadran(7, VUE_CRS_NB_LINES, 1, "STC", _imkstr((int)current_ma), "mA");
		this->cadran(7, VUE_CRS_NB_LINES, 2, "SOC", has_stc ? _imkstr((int)percentageBatt(voltage_v, current_ma)) : "--", "%");
...
void VueCRS::afficheScreen2(void) {
	...
	this->cadran(5, VUE_CRS_NB_LINES, 1, "Next turn", _imkstr(m_komoot_nav.distance), "m");
	this->cadranRR(5, VUE_CRS_NB_LINES, 2, "RR", rrZones);

	const uint8_t* bitmap = komoot_nav_get_icon(m_komoot_nav.direction);
	if (bitmap) {
...
void VueCRS::afficheSensors(void) {
	...
	// GFX port Phase D: see this file's top-of-file note -- FXOS8700
	// sensor fusion (yaw/pitch/roughness/histogram) isn't ported yet, so
	// there is no real data source for this page. A plain, honest
	// placeholder instead of either fabricating values or leaving the
	// screen blank with no explanation.
	this->setTextSize(2);
	this->setCursor(10, 100);
	this->print("Sensors page");
	this->setCursor(10, 130);
	this->print("(FXOS not ported)");
}
...
void VueCRS::afficheSegment(uint8_t ligne, Segment *p_seg) {
	...
	if (ligne < VUE_CRS_NB_LINES) drawFastHLine(0, fin_cadran, _width, 0);
	...
	if (p_seg->getStatus() == SEG_OFF) {
		...
		LOG_DEBUG("VueCRS delta1 lon: %ld", (int)(delta._x * deglon_to_m));
		LOG_DEBUG("VueCRS delta1 lat: %ld", (int)(delta._y * deglat_to_m));
		...
		LOG_DEBUG("Printing SEG status=%d", p_seg->getStatus());

		LOG_DEBUG("VueCRS delta2 lon: %ld", (int)((maxLon - minLon) * deglon_to_m));
		LOG_DEBUG("VueCRS delta2 lat: %ld", (int)((maxLat - minLat) * deglat_to_m));
	...
	LOG_DEBUG("VueCRS %u points printed", points_nb);
	...
	if (att.loc.course > 0) {
		...
		fillCircle((int16_t)maDpex, (int16_t)maDpey, 4, 0);
	}
...
void VueCRS::partner(uint8_t ligne, Segment *p_seg) {
	...
	fillRect(ol, hl, _width - 2 * ol, 3, 0);
	...
	drawFastVLine(_width / 2 - dixP, hl + 2, 7, 0);
	drawFastVLine(_width / 2 + dixP, hl + 2, 7, 0);

	fillTriangle(centre - 7, hl + 7, centre, hl - 7, centre + 7, hl + 7, 0);
