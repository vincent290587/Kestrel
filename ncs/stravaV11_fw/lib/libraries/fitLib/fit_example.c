...
/* Ride-recording feature (2026-09-11): NULLed out every entry this port
 * doesn't actually reference (only file_id/file_creator/event/record/
 * lap/session/activity are ever passed to Fit_InitMesg()/ride_write_mesg()
 * in ride_recorder.c) -- array *positions* are untouched, so every
 * FIT_MESG_* enum value used elsewhere still resolves correctly, but with
 * nothing left pointing at the other ~58 message-def structs, the linker's
 * existing -ffunction-sections/-fdata-sections + --gc-sections (already on
 * for this build) drops their now-dead const data instead of linking in
 * the whole profile's worth of it. Same "hand-trim the auto-generated
 * profile to only what's used" precedent this project's own
 * FIT_RECORD_FIELDS_IN_USE trim in fit_example.h already set, needed here
 * because the resulting image was ~600B over MCUboot's true max size (see
 * pm_static_stravav11_nrf52840.yml) even after maxing out partition
 * headroom and consolidating ride_recorder.c's own repeated code. */
const FIT_CONST_MESG_DEF_PTR fit_mesg_defs[] =
{
   (FIT_CONST_MESG_DEF_PTR) FIT_NULL,
