 ...
 * POINT_FMT "<hhh") -- kept as plain byte offsets/sizes here rather than
...
#define POINT_SIZE         6  /* i16 dlat + i16 dlon + i16 alt_m */
			...
			uint16_t index, float *lat, float *lon, float *alt)
{
	...
	int16_t alt_m = (int16_t)sys_get_le16(it->buf + off + 4);

	*lat = (float)((double)it->origin_lat_e6 / 1e6 + (double)dlat / 1e6);
	*lon = (float)((double)it->origin_lon_e6 / 1e6 + (double)dlon / 1e6);
	*alt = (float)alt_m; /* MAP_TILE_ALT_UNKNOWN_M passed through as-is, not NAN */
}
