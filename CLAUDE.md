...
- GPS UART: **`RX poll clean, 7 bytes received`** — every DK run showed 0 (nothing attached); something real answered the `$PMTK220` command on this board. Not decoded by the demo yet — worth a closer look whenever GPS bring-up resumes, but a genuinely new, unexplained-so-far signal worth flagging rather than a validated finding.
...
**Not yet done**: wiring GPS_R/GPS_S/FIX_PIN and the NEO_PIN status LED into devicetree; decoding what's producing the 7 bytes on the GPS UART; STC3100/VEML6075 custom drivers (unchanged gap from Phase 3); actually pairing a real ANT+ sensor or BLE Cycling Power peripheral; a `.yaml` twister/hardware-map metadata file for the new board (not needed for a plain build, only for test filtering).
