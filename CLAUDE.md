...
**What's actually validated, and what isn't yet**: unlike every other "nothing attached" peripheral in this port, this one has a real, checkable host-visible signal — a new USB device enumerating on this machine — but it needs a physical USB cable in the DK's *own* USB connector (separate from the J-Link connector already in use for flashing/console), which this session can't plug in itself. What **is** confirmed on real hardware: the on-die USB device controller initializes correctly (`udc_nrf: Preinit` / `udc_nrf: Initialized`), the USBD stack builds a valid descriptor set (`bNumInterfaces 2 wTotalLength 75`), and everything from Phases 2-8 keeps running correctly alongside it. `lsusb` on this machine shows no new device beyond the existing SEGGER J-Link, confirming the second cable isn't connected yet — real host-enumeration is the one remaining check, gated on that physical connection.
