...
Not done yet: `ble_lns_c` (Location and Navigation Service) and `ble_komoot_c` (custom Komoot navigation service) clients, and wiring any of these into the ported `Model`/`Boucle*` logic from Phase 1 (that integration is its own later step, same as the driver-to-logic wiring gap noted in earlier phases).
